...
If two values tie, both values are the mode and the distribution is [bimodal]{.defword}`r tufte::margin_note("A **bimodal** distribution has two modes.")`. Sometimes a distribution has two distinct clusters, each with its own local mode. The greater of these two modes is the *major mode*, and the lesser is the *minor mode* (See Figure&nbsp;\@ref(fig:bimodal)).
...
map2(X, reps, rep) %>% unlist %>% 
  ...
  mutate(cf = cumsum(f),
    ...
    mutate(across(.cols = p:cp, 
                .fns = function(x) scales::number(x, .01) %>% 
                  str_remove("^0"))) %>%
  knitr::kable(caption = "Finding the Median in a Frequency Distribution Table.<br>In this case, the median is 5 because it has the first cumulative proportion that is greater than 0.5.", 
               digits = 2, 
               align = "r",
