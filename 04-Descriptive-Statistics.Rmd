...
It is also common to supplement frequency distribution tables with additional information such as the [cumulative frequency]{.defword}`r tufte::margin_note("The **cumulative frequency** tells us the number of scores in a distribution that are equal to or lower than a particular sample space element.")`. For each sample space element, the cumulative frequency $(cf)$ is the sum of the frequencies $(f)$ of the current and all previous sample space elements.
...
```{r freqtablepubCode, eval=FALSE, echo=TRUE, ref.label="freqtablepub"}
...
```{r freqbarplot, fig.height=7, fig.cap="Frequency Distribution Bar Plot"}
d %>% 
  rename(Anger = anger) %>% 
  count(Anger, name = "f") %>% 
  mutate(cf = cumsum(f),
  ...
  geom_text_fill(aes(label = f), vjust = -0.5) +
  scale_x_continuous(breaks = 1:7, minor_breaks = NULL) + 
  scale_y_continuous("Frequency") +
  theme(panel.grid.major.x = element_blank())       
```

```{r freqstepplot, fig.cap="Cumulative Frequency Step Plot", fig.height=7}
d %>% 
  rename(Anger = anger) %>% 
  count(Anger, name = "f") %>% 
  mutate(cf = cumsum(f),
  ...
  geom_text_fill(aes(label = cf), vjust = -0.5) + 
  geom_text_fill(aes(label = signs::signs(f, accuracy = 1, add_plusses = T), 
                     y = cf - f / 2)) + 
  scale_x_continuous(breaks = 1:7, minor_breaks = NULL) + 
  scale_y_continuous("Cumulative Frequency", limits = c(0, NA))
```


```{r, fig.width=2, fig.margin = TRUE, fig.align='default', warning=FALSE, fig.cap="Cumulative Frequency Stacked Bar Plot"}
d_freq <- d %>% 
  rename(Anger = anger) %>% 
  count(Anger, name = "f") %>% 
  mutate(cf = cumsum(f),
         ...
         cp = cumsum(p))
d_freq %>% 
  ggplot(aes(factor("Anger"), cp - p / 2)) + 
  geom_tile(aes(height = p, fill = factor(Anger)), 
            width = 1.2) +
...
The [mode]{.defword}`r tufte::margin_note("The **mode** is the value in a distribution that occurs most often.")` is the most frequent score in a distribution. Suppose we have a distribution that looks like this: 
...
In a histogram or probability density plot, the mode is the value that corresponds to the highest point in the plot. If two values tie, both values are the mode and the distribution is [bimodal]{.defword}`r tufte::margin_note("A **bimodal** distribution has two modes.")`. Sometimes a distribution has two distinct clusters, each with its own local mode. The greater of these two modes is the *major mode*, and the lesser is the *minor mode* (See Figure&nbsp;\@ref(fig:bimodal)).
...
```{r bimodalCode, ref.label='bimodal', echo=TRUE, eval=FALSE}
...
map2(X, reps, rep) %>% unlist %>% 
  ...
  mutate(cf = cumsum(f),
    ...
    mutate(across(.cols = p:cp, 
                .fns = function(x) scales::number(x, .01) %>% 
                  str_remove("^0"))) %>%
  knitr::kable(caption = "Finding the Median in a Frequency Distribution Table.<br>In this case, the median if 5 because it has the first cumulative proportion that is greater than 0.5.", 
               digits = 2, 
               align = "r",
               col.names = c("*X*","Frequency", "Cumulative Frequency" ,"Proportion", "Cumulative Proportion"))
```
...
The [arithmetic mean]{.defword}`r tufte::margin_note("The **arithmetic mean** is the balance point of a disribution.")` is the sum of all values of a distribution divided by the size of the distribution.
...
```{r centraltendency, eval=FALSE}
md <- function(i) {
  round(rchisq(100,4),digits = 2)
}
...
tibble(i = 1:1000) %>% 
  mutate(m = map(i, md)) %>% 
  mutate(mean = map_dbl(m, mean),
         median = map_dbl(m, median),
         mode = map_dbl(m, mmode)) %>% 
  select(mean, median, mode) %>% 
  gather("CentralTendency","Value") %>% 
  ggplot(aes(y = Value, x = CentralTendency)) +
  # geom_density(color = NA, alpha = 0.5) + 
  # stat_function(fun = function(x) 40 * dchisq(x, df = 40)) + 
  # scale_x_continuous(limits = c(10,80))
  ggbeeswarm::geom_quasirandom(alpha = 0.5, pch = 16, size = 0.1) +
  stat_summary(color = "firebrick", fun.data = mean_sdl, fun.args = list(mult = 1)) +
  scale_y_continuous(NULL, minor_breaks = seq(0,10000,5))
...
At one level, the concept of the [expected value]{.defword}`r tufte::margin_note("The **expected value** of a random variable is the population mean of the values that the random variable generates.")` of a random variable is really simple; it is just the population mean of the variable. So why don't we just talk about population means and be done with this "expected value" business? It just complicates things! True. In this case, however, there is value in letting some simple things appear to become complicated for a while so that later we can show that some apparently complicated things are actually simple.
...
tibble(x = factor(c(2,4,8), levels = 1:8),
  ...
  ggplot(aes(x,p)) + 
  geom_col(fill = myfills[1]) + 
  geom_text(aes(label = WJSmisc::remove_leading_zero(p)), 
            vjust = -0.4, 
            family = bfont, 
            size = 6) + 
  ...
  scale_y_continuous("Probability", 
                     ...
                     breaks = seq(0,1,.1),
                     labels = WJSmisc::remove_leading_zero
                     ) + 
...
```{r thinbinsCode, echo=T, ref.label="thinbins", eval=F}
...
```{r nominalvarCode, ref.label="nominalvar", echo=TRUE, eval=FALSE}
